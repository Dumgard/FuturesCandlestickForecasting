...
def deepcopy(a):
    ...
    for el in a:
        if el is None:
            ...
            b.append(el.clone().detach())
        elif isinstance(el, list) or isinstance(el, tuple):
            b.append(deepcopy(el))
        else:
            print("Strange condition: el =", el, "in a....\n")
    return b


class HFWrapper:
    def __init__(self, model, criterion, optimizer, use_DAIN=False):
        self.model = model
        self.criterion = criterion
        self.opt = optimizer
        self._params_list = [p for p in self.opt._params]
        self.closure = None
        self.state = deepcopy(model.rnn_states)
        self.dain = use_DAIN

    def reset_grads(self):
        for p in self.opt._params:
            p.grad = None

    def step(self, X, Y, mean=None, std=None):
        def closure():
            self.model.rnn_states = deepcopy(self.state)
            if self.dain:
                Y_hat = self.model(X, mean, std)
            else:
                Y_hat = self.model(X)
        ...
        self.state = deepcopy(self.model.rnn_states)
        return loss
...
class HessianFree(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        cg_max_iter (int, optional): Maximum number of Conjugate-Gradient
    ...
    def _gather_flat_grad(self, loss):
    ...
    # def _gather_flat_grad_old(self):
    #     views = list()
    ...
    def step(self, closure, b=None, M_inv=None):
        ...
        # print(self._gather_flat_grad(loss_before))
        # print(self._gather_flat_grad_old())
        # print(self._gather_flat_grad_old() != self._gather_flat_grad(loss_before))
        # print(torch.any(self._gather_flat_grad_old() != self._gather_flat_grad(loss_before)))
        flat_grad = self._gather_flat_grad(loss_before)
